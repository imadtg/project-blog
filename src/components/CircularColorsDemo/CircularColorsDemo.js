"use client";
import React from "react";
import clsx from "clsx";
import { Play, Pause, RotateCcw } from "react-feather";
import { motion } from "framer-motion";

import Card from "@/components/Card";
import VisuallyHidden from "@/components/VisuallyHidden";

import styles from "./CircularColorsDemo.module.css";

const COLORS = [
  { label: "red", value: "hsl(348deg 100% 60%)" },
  { label: "yellow", value: "hsl(50deg 100% 55%)" },
  { label: "blue", value: "hsl(235deg 100% 65%)" },
];

function useTimeElapsed(
  initiallyPaused = true,
  initialValue = 0,
  intervalLength = 1000
) {
  const [timeElapsed, setTimeElapsed] = React.useState(initialValue);
  const [isPaused, setIsPaused] = React.useState(initiallyPaused);
  React.useEffect(() => {
    if (isPaused) {
      return;
    }
    const intervalId = window.setInterval(
      () => setTimeElapsed((previousTimeElapsed) => previousTimeElapsed + 1),
      intervalLength
    );

    return () => {
      window.clearInterval(intervalId);
    };
  }, [intervalLength, isPaused]);
  function resetTimeElapsed(pauseAfterReset = initiallyPaused) {
    setTimeElapsed(initialValue);
    if (pauseAfterReset) {
      setIsPaused(true);
    }
  }
  function toggleTimeLapse() {
    if (isPaused) {
      setTimeElapsed(timeElapsed + 1);
    }
    setIsPaused(!isPaused);
  }
  return {
    timeElapsed,
    isPaused,
    resetTimeElapsed,
    toggleTimeLapse,
    setIsPaused,
  };
}

function CircularColorsDemo() {
  const { timeElapsed, isPaused, resetTimeElapsed, toggleTimeLapse } =
    useTimeElapsed();
  const id = React.useId();

  const selectedColorIndex = timeElapsed % COLORS.length;

  const selectedColor = COLORS[selectedColorIndex];

  return (
    <Card as="section" className={styles.wrapper}>
      <ul className={styles.colorsWrapper}>
        {COLORS.map((color, index) => {
          const isSelected = color.value === selectedColor.value;

          return (
            <li className={styles.color} key={index}>
              {isSelected && (
                <motion.div
                  layoutId={`${id}-color-outline`}
                  className={styles.selectedColorOutline}
                />
              )}
              <div
                className={clsx(
                  styles.colorBox,
                  isSelected && styles.selectedColorBox
                )}
                style={{
                  backgroundColor: color.value,
                }}
              >
                <VisuallyHidden>{color.label}</VisuallyHidden>
              </div>
            </li>
          );
        })}
      </ul>

      <div className={styles.timeWrapper}>
        <dl className={styles.timeDisplay}>
          <dt>Time Elapsed</dt>
          <dd>{timeElapsed}</dd>
        </dl>
        <div className={styles.actions}>
          <button onClick={toggleTimeLapse}>
            {isPaused ? (
              <>
                <Play />
                <VisuallyHidden>Play</VisuallyHidden>
              </>
            ) : (
              <>
                <Pause />
                <VisuallyHidden>Pause</VisuallyHidden>
              </>
            )}
          </button>
          <button onClick={resetTimeElapsed}>
            <RotateCcw />
            <VisuallyHidden>Reset</VisuallyHidden>
          </button>
        </div>
      </div>
    </Card>
  );
}

export default CircularColorsDemo;
